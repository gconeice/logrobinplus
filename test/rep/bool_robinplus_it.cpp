...
void test_circuit_zk(BoolIO<NetIO> *ios[threads], int party, size_t branch_size, size_t nin, size_t nx) {
    ...
    if (party == ALICE) {
        for (size_t bid = 0; bid < branch_size; bid++) M.push_back( cir[0].robinplus_acc_P(com_in, com_o, alpha_power, final_res) );
        assert( M[id].coef[2] == gf128bit::zero() );
    } else {
        for (size_t bid = 0; bid < branch_size; bid++) K.push_back( cir[0].robinplus_acc_V(com_in, com_o, alpha_power, final_res, gf128bit(delta)) );
    }
    ...
    prove_product_zero_it(ios, party, com_Q);

    // V (Bob) issues random challenge to randomly combine quadratic equation
    // Bob issues random challanges alpha and parties compute its powers
    gf128bit beta;
    if (party == ALICE) {
		ios[0]->recv_data(&beta, sizeof(gf128bit));
    } else {
		PRG().random_data(&beta, sizeof(gf128bit));
		ios[0]->send_data(&beta, sizeof(gf128bit));	        
        ios[0]->flush();
    }    
    gf128bitTriple acc_M;
    gf128bit acc_K;
    gf128bit beta_coeff = gf128bit::unit();
    ext_f2 acc_Q;
    if (party == ALICE) {
