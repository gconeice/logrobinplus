...
void test_circuit_zk(BoolIO<NetIO> *ios[threads], int party, size_t branch_size, size_t nin, size_t nx) {
    ...
    for (size_t bid = 0; bid < branch_size; bid++) bmac.push_back( cir[bid].robin_acc(com_in, com_l, com_r, com_o, alpha_power, final_res.minor()) );

    for (size_t bid = 0; bid < branch_size; bid++)
        std::cout << bid << ' ' << HIGH64(bmac[bid].key.val) << LOW64(bmac[bid].key.val) << ' ' << HIGH64(bmac[bid].val.val) << LOW64(bmac[bid].val.val) << std::endl;
