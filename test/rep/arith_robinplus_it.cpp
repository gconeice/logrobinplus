...
void test_circuit_zk(BoolIO<NetIO> *ios[threads], int party, size_t branch_size, size_t nin, size_t nx) {
    ...
    for (size_t i = 0; i < nx; i++) alpha_power.push_back( alpha_power.back() * f61(alpha) );
    ...
    uint64_t beta;
    if (party == ALICE) {
		ZKFpExec::zk_exec->recv_data(&beta, sizeof(uint64_t));
        beta = beta % PR; // to prevent cheating V
    } else {
		PRG().random_data(&beta, sizeof(uint64_t));
		beta = beta % PR;
		ZKFpExec::zk_exec->send_data(&beta, sizeof(uint64_t));	        
    }    
    f61Triple acc_M; acc_M.coef[2] = acc_M.coef[1] = acc_M.coef[0] = f61::zero();
    f61 acc_K = f61::zero();
    f61 beta_coeff = f61::unit();
